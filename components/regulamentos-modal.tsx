...
import { FileText, ChevronRight, ArrowLeft, Check } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const regulamentos: Regulamento[] = [
  {
    id: "geral",
    title: "Regulamento Geral",
    description: "Regras gerais da AMIGOS DA MESA PROP",
    content: `REGULAMENTO GERAL – AMIGOS DA MESA PROP
...
A aquisição de quaisquer planos, exames ou modalidades ocorre exclusivamente por meio do site oficial www.amigosdamesaprop.com, implicando aceitação integral, irrestrita e irrevogável deste Regulamento.
...
Os exames, simuladores remunerados e contas reais são realizados por meio da plataforma Profit, da Nelogica, em suas versões ONE, PLUS ou PRO, sendo a versão padrão o Profit ONE.
...
O exame será realizado em conta simulador.
Não é permitida troca de plano ou plataforma após o início do teste.
A pausa das contas é permitida por até 30 (trinta) dias, mediante comunicação prévia por e-mail.
Não há dias mínimos nem máximos operados para aprovação, desde que a plataforma esteja devidamente paga mensalmente. A responsabilidade de entrar em contato com a mesa para efetuar o pagamento é do Trader e o não pagamento pode acarretara em cancelamento, taxas adicionais e outros.
...
Uma mesa para todos.`,
  },
  {
    id: "guia-rapido",
    title: "Guia Rápido",
    description: "Como funciona a AMIGOS DA MESA PROP",
    content: `GUIA RÁPIDO – COMO FUNCIONA A AMIGOS DA MESA PROP
...
Limite operacional no SR: Em um mesmo pregão, o Trader não pode ultrapassar 50% do saldo total de loss (stop global) do plano e caso ocorra o dia será eliminado.
Essa regra existe para evitar:
• overtrade
• alavancagem excessiva
• ganhos fora do perfil do plano
Caso esse limite seja ultrapassado, o valor excedente será desconsiderado, conforme avaliação da mesa.
Essa limitação aplica-se exclusivamente ao Simulador Remunerado.
...
• Permanecem válidos:
  • stop diário
  • saldo total de loss (stop global)
  • regras de conduta
...
Exemplo simples:
• Stop Global do plano: R$ 5.000
• Ao longo do período, o Trader atinge R$ 5.000 em perdas → A conta é eliminada no momento em que esse limite é alcançado.
...
Este material tem caráter exclusivamente informativo, servindo como apoio à compreensão das principais regras e fluxos operacionais. As condições completas, critérios oficiais e decisões finais permanecem integralmente regidas pelo Regulamento Geral vigente.
A atuação do Trader pressupõe a leitura, ciência e concordância com as regras aplicáveis, sendo sua responsabilidade operar de forma consciente, responsável e alinhada às diretrizes da mesa.`,
  },
  {
    id: "material-1",
    title: "Material Explicativo I",
    description: "Planos Master50 / Unos40 / Titan / Prime / Pegue e Monte / BIT",
    content: `MATERIAL EXPLICATIVO COMPLEMENTAR I
...
• Caso atinja 50% de perda da margem em Conta Real, perderá também qualquer saldo acumulado previamente no Simulador Remunerado
...
As regras acima aplicam-se integralmente aos planos Master50, Unos40, Titan, Pegue e Monte 27, Pegue e Monte 32, Prime16 e Prime21, Bit20 e Bit30, prevalecendo sobre disposições gerais quando aplicáveis.`,
  ...
  {
    id: "material-2",
    title: "Material Explicativo II",
    description: "Demais Planos (Iniciante, Intermediário, Avançado, Prime, Pegue e Monte)",
    content: `MATERIAL EXPLICATIVO COMPLEMENTAR II
...
Se atingir 50% de perda do saldo global em Conta Real, perderá também qualquer saldo acumulado restante no Simulador Remunerado.
...
As regras descritas neste material aplicam-se exclusivamente aos planos aqui mencionados, não se confundindo com as regras específicas dos planos Master50, Unos40 e Titan.`,
  ...
  {
    id: "campanhas",
    title: "Campanhas Acima de 50% OFF",
    description: "Modalidade de Saque Mensal para campanhas promocionais",
    content: `REGULAMENTO ESPECÍFICO – CAMPANHAS ACIMA DE 50% OFF
...
Os saques nesta modalidade ocorrerão exclusivamente em ciclos mensais, com fechamento nos dias 30 ou 31 de cada mês.
...
b) após este período, o repasse será de 95% (noventa e cinco por cento).
...
export function RegulamentosModal() {
  ...
  const [readRegulamentos, setReadRegulamentos] = useState<Set<string>>(new Set())
  const [acceptedAll, setAcceptedAll] = useState(false)

  const allRead = regulamentos.every((r) => readRegulamentos.has(r.id))

  const handleSelectRegulamento = (reg: Regulamento) => {
    setSelectedRegulamento(reg)
    setReadRegulamentos((prev) => new Set(prev).add(reg.id))
  }
  ...
  const handleOpenChange = (isOpen: boolean) => {
    // Só permite fechar se já aceitou todos os termos ou se está fechando pela primeira vez sem ter aberto
    if (!isOpen && !acceptedAll && open) {
      // Não permite fechar se não aceitou
      return
    }
    setOpen(isOpen)
    if (!isOpen) {
  ...
  const canClose = acceptedAll || !open
        ...
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            {selectedRegulamento ? (
              ...
              "Regulamentos da Mesa"
            )}
          </DialogTitle>
          <DialogDescription>
            {selectedRegulamento
              ? selectedRegulamento.description
              : "Leia atentamente todos os regulamentos antes de prosseguir. Você deve ler todos os documentos para concordar com os termos."}
          </DialogDescription>
        </DialogHeader>

        {selectedRegulamento ? (
          /* Conteúdo do regulamento selecionado */
          <div className="flex-1 overflow-y-auto pr-2 min-h-0">
            <div className="space-y-4 text-sm text-muted-foreground whitespace-pre-line leading-relaxed">
              {selectedRegulamento.content}
            </div>
          </div>
        ) : (
          ...
          <div className="flex-1 overflow-y-auto pr-2 min-h-0">
            <div className="space-y-2">
              {regulamentos.map((reg) => {
                const isRead = readRegulamentos.has(reg.id)
                return (
                  <button
                    key={reg.id}
                    onClick={() => handleSelectRegulamento(reg)}
                    className={`
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">{reg.description}</p>
                      </div>
        ...
        <div className="flex-shrink-0 pt-4 border-t border-border">
          {selectedRegulamento ? (
            <Button onClick={handleBack} variant="outline" className="w-full">
              ...
              Voltar para lista de regulamentos
            </Button>
          ) : (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">
                  Regulamentos lidos: {readRegulamentos.size} de {regulamentos.length}
                </span>
                {allRead && (
              ...
              >
                {allRead ? "Li e concordo com todos os termos" : "Leia todos os regulamentos para continuar"}
              </Button>
              {!allRead && (
                <p className="text-xs text-muted-foreground text-center">
                  Você precisa ler todos os regulamentos antes de concordar e fechar.
                </p>
              )}
            </div>
          )}
